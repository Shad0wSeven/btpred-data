...
ticker=${BTPRED_KALSHI_PERP_TICKER:-KXBTCPERP}
python_bin=${BTPRED_PYTHON:-python3}
data_dir="$publish_repo/kalshi_perp"
...
relative_path="kalshi_perp/${ticker}-${started}-${chunk_seconds}s.jsonl.gz"
output_path="$publish_repo/$relative_path"

"$python_bin" "$source_repo/scripts/capture_kalshi_perp.py" \
  --ticker "$ticker" \
...
git -C "$publish_repo" commit --only \
  -m "Add ${ticker} reference and book capture ${started}" \
  -- "$relative_path"
